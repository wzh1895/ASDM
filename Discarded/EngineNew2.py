...
class Structure(object):
    ...
    def process_equation(self, input_str, default_element=None): # a[1, b] --> a__ele1__1__cmm__b__ele2__
        ...
        output_str = input_str.replace('[', '__ele1__').replace(']', '__ele2__').replace(',', '__cmm__')
    ...
    def compile(self):
        ...
        # for var_element, compiled_equation in self.var_eqns.items():
        ...
        for var, equation in self.var_eqns.items():
            if type(equation) is str:
                print('ccc0', var, equation)
                equation = self.process_equation(equation)
                self.var_eqns_compiled[var] = equation

            elif type(equation) is dict:
                print('ccc1', var, equation)
                self.var_eqns_compiled[var] = dict()
                for ele, eqn in equation.items():
                    self.var_eqns_compiled[var][self.process_element(ele)] = self.process_equation(eqn)
    ...
    def calculate(self, expression, gb, lc, element=None):
        print('eee')
        pprint(expression)
        try:
            print('eee0', type(expression))
            pprint(expression)
            ...
            if type(expression) is dict:
                value = dict()
                for element, ele_expression in expression.items():
                    print('element: {}, expression: {}'.format(element, ele_expression))
                    value[element] = self.calculate(ele_expression, gb=gb, lc=lc, element=element) 
                    # Problem: a/b , a and b both dict, a and b both in lc (as dict), 
                    # there is no way to interupt eval()'s retrival of a and b. 
                    # the design of lc (instead of relying on Exception handling causes this problem)
            else:
            ...
            value = self.calculate(new_expression, gb=gb, lc=lc)
    ...
    def init_stock(self, var, diagram, calculated, if_stock):
        ...
        try:
            ...
            for element in elements:
                # var_ele = var+element
                var_ele_eqn = self.var_eqns_compiled[var][element]
                # print('iii1', var, var_ele, var_ele_eqn)
                ...
                var_ele_v = self.calculate(var_ele_eqn, gb={}, lc=calculated)
                
                try:
                    calculated[var][element] = var_ele_v
                except KeyError:
                    calculated[var] = dict()
                    calculated[var][element] = var_ele_v
                ...
                if if_stock:
                    self.var_eqns_runtime[var][element] = str(var_ele_v) # str needed for eval()
            ...
            var_v = self.calculate(var_eqn, gb={}, lc=calculated)
    ...
    def update_flow_aux(self, var, diagram, calculated=dict()):
        ...
        try:
            ...
            for element in elements:
                # var_ele = var+element
                var_ele_eqn = self.var_eqns_runtime[var][element]

                for dependency in dependencies:
                    self.update_flow_aux(dependency, diagram, calculated)

                # print('ufa1.1', var_ele_eqn)
                var_ele_v = self.calculate(var_ele_eqn, gb=self.var_eqns_runtime, lc=calculated)
            ...
            var_v = self.calculate(var_eqn, gb=self.var_eqns_runtime, lc=calculated)
    ...
    def simulate(self, simulation_time=None, dt=None):
        ...
        for step in range(total_steps):
            ...
            self.var_eqns_runtime = self.current_time
            print('\nSTEP {}\n'.format(self.current_step))
