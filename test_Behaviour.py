...
def test(test_path, outcome_path):
    ...
    # Load reference outcome
    df_reference = pd.read_csv(outcome_path)
    ...
    df_reference.drop([columns[0]], axis=1, inplace=True)

    reference_outcome = df_reference.rename(columns=column_name_map)
    reference_outcome = reference_outcome[sorted(reference_outcome.columns)] 

    # Comparison by subtracting
    df_comparison = asdm_outcome.subtract(reference_outcome, axis=1)
