...
class Parser(object):
    def __init__(self):
        ...
        self.patterns_type = {
            'INT__LPAREN__FUNC__RPAREN':{
                'token':['INT', 'FUNC'],
                'operator':['INT'],
                'operand':['FUNC']
            },

            'INT__LPAREN__NUMBER__RPAREN':{
                'token':['INT', 'NUMBER'],
                'operator':['INT'],
                'operand':['NUMBER']
            }
    ...
    def parse(self, string, verbose=False, max_iter=100):
        ...
        while len(items) > 1 and r > 0:
            ...
            """ 
            The order of the following patterns is IMPORTANT
            The patterns that come first are the ones that are more specific, so they are prioritised in parsing
            """
            for pattern_set in [ 
                self.patterns_sub_var,
                self.patterns_num,
                    ...
                    for invalid_var in invalid_vars:
                        ...
                        if type(asdm_result) is dict:
                            for sub, subval in asdm_result.items():
                                print('Reference result:', debug[self.var_name_to_csv_entry(invalid_var+'[{}]'.format(', '.join(sub)))])
                        else:
                            print('Reference result:', debug[self.var_name_to_csv_entry(invalid_var)])
                        print('\n')
